...
* Fixed
  ...
  * `Register-DiscoveryScheduledTask.ps1` + `Copy-WUGDashboardReports.ps1` -- Scheduled tasks created by the wizard exited immediately; two issues: (1) the `-Command` wrapper spawned a nested `powershell.exe -File "C:\Program Files\..."` whose double-quoted path broke the outer `-Command "..."` string causing Task Scheduler to truncate the command, and (2) `try { }; finally { }` broke because the semicolon between `}` and `finally` terminates the try statement; replaced entire `-Command "one-liner"` approach with `-EncodedCommand` (Base64) which eliminates all quoting and syntax issues
  * 7 `Setup-*-Discovery.ps1` scripts (Proxmox, Azure, OCI, Bigleaf, Nutanix, GCP, Docker) -- Push-to-WUG device creation silently swallowed API errors and falsely reported devices as "created"; `Add-WUGDeviceTemplate` returns an error array with `.messages` properties on failure, but the calling code checked `$devResult.error` (singular, wrong property name) which never matched; also `$stats.DevicesCreated++` incremented outside the device-ID check so failures counted as successes; fixed error detection to inspect result type (array = error), moved counter inside `if ($newDeviceId)`, and added `Write-Warning` with actual API error messages
  * `Start-WUGDiscoverySetup.ps1` -- Fixed GCP credential vault name from `GCP.KeyFile` to `GCP.ServiceAccount` to match `Setup-GCP-Discovery.ps1`
  * `Invoke-WUGDiscoveryRunner.ps1` -- Fixed provider config mismatches: VMware `DashboardFunc` (`Export-VMwareDashboardHtml` → `Export-VMwareDiscoveryDashboardHtml`); GCP VaultName (`GCP.Credential` → `GCP.ServiceAccount`) and CredType (`GCPServiceAccount` → `FilePath`); vault names for Fortinet (`FortiGate-FW1` → `FortiGate.fw1.corp.local`), Nutanix (`Nutanix.Credential` → `Nutanix.nutanix-cluster.corp.local.Credential`), OCI (`OCI.Credential` → `OCI.Config`), Windows (`Windows.WMI.Credential` → `Windows.WMI.Credential.1`); all now match what the corresponding Setup scripts create
...
* Added
  ...
  * `DiscoveryHelpers.ps1` -- SNMPv2 and SNMPv3 credential types: new `Resolve-DiscoveryCredential` cases with interactive prompts (community string, username, auth/privacy protocol/password); `ConvertFrom-VaultStored` extended for `SNMPv2` and `SNMPv3` hashtable formats; auto-detection of `SNMPv2`/`SNMPv3` CredType from vault name patterns (`*.Snmp$`)
  * `DiscoveryHelpers.ps1` -- LocalMachine DPAPI integrity fix: integrity hash now uses `LOCALMACHINE` constant instead of user identity when scope is `LocalMachine`, preventing integrity check failures when credentials are read by a different user (e.g., SYSTEM) than the one that saved them
  * `Initialize-WUGDiscoveryVault.ps1` -- **New script**: unified one-command vault initializer for all discovery providers (Azure, AWS, Proxmox, LoadMaster, Windows, CiscoWLC, CUCM); sets LocalMachine scope by default (SYSTEM-accessible); guides through credential entry per provider; prints or auto-registers scheduled tasks; numeric SNMPv3 auth/priv protocol inputs are mapped to named strings (e.g., `2` → `SHA`, `2` → `AES128`) before vault storage
  * `Register-DiscoveryScheduledTask.ps1` -- `-SkipVaultPopulate` switch: separates vault population from task registration; when combined with `-UseSystemVault`, registers task as SYSTEM without running interactive credential prompts (for use after `Initialize-WUGDiscoveryVault.ps1`); default output path now uses `C:\ProgramData\WhatsUpGoldPS\Output` (ProgramData) when `-UseSystemVault` is set, since SYSTEM cannot access `%LOCALAPPDATA%`; added `ApiKey` to `-AuthMethod` ValidateSet; displays `SYSTEM (LocalMachine vault)` in "Run As" field when `-UseSystemVault` is set
  * `Start-WUGDiscoverySetup.ps1` -- `-VaultScope [LocalMachine|CurrentUser]` parameter (default: `LocalMachine`); automatically falls back to `CurrentUser` with a warning if not running as Administrator; propagates `-UseSystemVault -SkipVaultPopulate` to `Register-DiscoveryScheduledTask.ps1` calls when `LocalMachine` scope is active; sets output path to `C:\ProgramData\WhatsUpGoldPS\Output` for LocalMachine scope
  * `Setup-Azure-Discovery.ps1` -- `[Alias('Target')]` on `-TenantId` so all providers can be called uniformly with `-Target`; credential extraction now handles both PSCredential format (legacy) and hashtable Fields-based vault storage; vault path respects `WUG_VAULT_SCOPE` environment variable
  * `Setup-AWS-Discovery.ps1` -- `[Alias('Target')]` on `-Region`; credential extraction handles both PSCredential and hashtable formats from Fields-based vault storage; removes redundant BSTR decryption in later sections
  * `Setup-OCI-Discovery.ps1` -- `[Alias('Target')]` on `-TenancyId` for uniform `-Target` calling convention
  * `Setup-LoadMaster-Discovery.ps1` -- Fixed dashboard output to use fixed filename `LoadMaster-Dashboard.html` (overwrites on each run) instead of timestamped filenames that accumulated without cleanup
  * `DiscoveryProvider-CUCM.ps1` -- End-of-MIB graceful handling: SharpSnmpLib bulk walk throws "empty array" on the final terminating packet (RFC-standard `endOfMibView`) which is not a real failure; catch block now detects this pattern and retains all collected data instead of discarding it and falling back to the WUG COM API
  ...
  * `Invoke-WUGDiscoveryRunner.ps1` -- `$RunWindowsAttributes` and `$RunWindowsDiskIO` parameters with full provider config, dispatch logic, and WMI credential handling
  * `Invoke-WUGDiscoveryE2ETest.ps1` -- LoadMaster provider: ValidateSet entries, `$LoadMasterTarget` parameter, default targets, vault config, and provider definition block
  * `Setup-WindowsAttributes-Discovery.ps1` + `Setup-WindowsDiskIO-Discovery.ps1` -- NmConsole dashboard copy block (detect WUG install path, copy HTML to `NmConsole\dashboards\`, call `Deploy-DashboardWebConfig`); all 15 providers now deploy dashboards to the WUG web console
  * 11 `Setup-*-Discovery.ps1` scripts (Bigleaf, Docker, GCP, HyperV, LoadMaster, Nutanix, OCI, Proxmox, VMware, WindowsAttributes, WindowsDiskIO) -- `NonInteractive` fallback: when running with `-NonInteractive` but no `-Action`, defaults to Dashboard (`$choice = '5'`) instead of hanging at `Read-Host`; F5, Fortinet, AWS, and Azure already had this safeguard
  * `Setup-F5-Discovery.ps1` + `Setup-Fortinet-Discovery.ps1` + `Setup-LoadMaster-Discovery.ps1` -- `-IncludeParams` on `Export-DiscoveryPlan -Format JSON` in ExportJSON action; all 12 standard providers now include operation parameters in JSON exports consistently
...
* Changed
  ...
  * `Connect-WUGServer.ps1` -- Integrated DPAPI credential vault; when called with no parameters, checks vault for saved WUG credentials and prompts to reuse/reset/new; successful connections auto-save to vault at `%LOCALAPPDATA%\WhatsUpGoldPS\DiscoveryHelpers\Vault`; warns when plaintext `-Password` parameter used on command line (PSReadLine history exposure); vault integrity now uses HMAC-SHA256 with DPAPI-protected key (was plain SHA-256); legacy SHA-256 fallback for pre-HMAC vault files; ACL restriction warning when directory permissions can't be set; **VaultSet now handles `-serverUri` without credentials**: auto-uses vault credentials when server matches, prompts for credentials when vault has a different server or is empty; **`-Password` parameter changed from mandatory to optional** in UserPassSet: when omitted, prompts via `Read-Host -AsSecureString` (masked input) instead of PowerShell's plaintext mandatory parameter prompt
...
* Fixed
  * Fixed `Connect-WUGServer -serverUri <host>` (without `-Credential`/`-Username`/`-Password`) returning 400 error -- VaultSet parameter set only checked vault when `-serverUri` was NOT provided; now checks vault for matching server credentials in all cases
  * Fixed `Connect-WUGServer -serverUri <host> -Username <user>` prompting for password in plaintext -- PowerShell's mandatory parameter prompt shows input unmasked; `-Password` is now optional and prompts via `Read-Host -AsSecureString` (masked with `*`) when omitted

...
function Add-WUGDevice {
    ...
    if ($SubRoles) { if ($SubRoles -is [array]) { foreach ($item in $SubRoles) { if ($item -isnot [string]) { throw "SubRoles parameter must be a one-dimensional array of strings." } } } else { throw "ActiveMonitors parameter must be a one-dimensional array of strings." } }
    ...
    if (!$Template) {
        $Template = @{
            ...
            subRoles            = (&{if(-not $SubRoles){@()}else{@(,$SubRoles)}})
            os                  = "${OS}"
            brand               = "${Brand}"

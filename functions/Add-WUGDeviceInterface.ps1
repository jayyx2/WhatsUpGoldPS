<#
.SYNOPSIS
Adds a network interface to a device in WhatsUp Gold.

.DESCRIPTION
The Add-WUGDeviceInterface function creates a new network interface on a device
using POST /api/v1/devices/{deviceId}/interfaces/-.
...
#>
function Add-WUGDeviceInterface {
    ...
    begin {
        Write-Debug "Starting Add-WUGDeviceInterface function."
    ...
    process {
        ...
        if (-not $PSCmdlet.ShouldProcess("Device ${DeviceId}", 'Add interface')) { return }
        ...
        try {
            ...
            Write-Error "Error adding interface to device ${DeviceId}: $_"
        }
    }

    end {
        Write-Debug "Completed Add-WUGDeviceInterface function."
    }
}

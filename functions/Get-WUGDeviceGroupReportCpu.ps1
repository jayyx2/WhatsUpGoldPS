<#
.SYNOPSIS
Retrieves CPU utilization reports for devices within specified groups from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceGroupReportCpu function fetches CPU utilization reports for all devices within the specified group IDs from WhatsUp Gold's API. It supports a range of filtering options, including time range, sorting, grouping, thresholds, and pagination. It also allows for the inclusion of devices from sub-groups within the specified groups.

.PARAMETER GroupId
An array of group IDs from which to fetch CPU utilization reports. This parameter is mandatory.
...
.EXAMPLE
PS> Get-WUGDeviceGroupReportCpu -GroupId @("1", "2") -Range "lastWeek" -SortBy "avgPercent" -SortByDir "desc" -Limit 50
Fetches last week's CPU utilization reports for devices within groups with IDs 1 and 2, sorted by average percent in descending order, with a limit of 50 entries per page.

$Cred = Get-Credential
Connect-WUGServer -serverUri 192.168.1.250 -Credential $Cred
$groups = Get-WUGDeviceGroups
Get-WUGDeviceGroupReportCpu -GroupId $groups

.NOTES
Author: Jason Alberino (jason@wug.ninja) 2024-03-29

Reference: https://docs.ipswitch.com/NM/WhatsUpGold2022_1/02_Guides/rest_api/#operation/DeviceGroupReport_DeviceCpuUtilizationReport
...
function Get-WUGDeviceGroupReportCpu {
    param (
        [array]$GroupId,
        ...
        [string]$RangeEndUtc,
        [int]$RangeN,
        [ValidateSet("defaultColumn", "id", "deviceName", "cpu", "cpuId", "pollTimeUtc", "timeFromLastPollSeconds", "minPercent", "maxPercent", "avgPercent")][string]$SortBy,
        [ValidateSet("asc", "desc")][string]$SortByDir = "desc",
        [ValidateSet("noGrouping", "id", "deviceName", "cpu", "cpuId", "pollTimeUtc", "timeFromLastPollSeconds", "minPercent", "maxPercent", "avgPercent")][string]$GroupBy,
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDvd4ajZO81vAcy
# OvlcLOZMVug4fUoIF3B3sO2N80u67aCCEfkwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BgkqhkiG9w0BCQQxIgQgjK3WlRGh3bo79qSBEkqYp9gFnAeZVFvQr4qkyJAszcUw
# DQYJKoZIhvcNAQEBBQAEggIAU2tKrHIam69xoJkvBKi6jxzqyUGOzSreuTBe0A2q
# y5pb6KOnqHkoBqdLHISdYK14wy5txpTXApDxgw0dXEyzXFePZTED/TiCcXjL8WU2
# JjuQ3IrUaM4HA4DED5BkYmqLiAgGPoc+ym4aZztB0FMDM9xKLcKXV/AOs9AmZIeO
# k/86K7Rx1BdhLS5s3H3KVR99s3rfkrPpFkuzXQANOAdw2WeIj+EiRdJja+a5e5I6
# NJ9mOf+u4yjDC+xqrSiInN6ANpJfp3gbgXPbMMEhokbLuNVktQO0hMSKa+HGarMf
# 9Dv4uJDCqXrElWDLZIIQ2Ah2Ml0D+Ys11h11mjYk+4NvMB+WCCL0o/du/xAwIkUw
# jJK/5igsjuL5I1165eT6bOI1jARiyTKfn4rljsQl07bNFcEtDsm4LjJU8zJSaHbh
# HJVaOEbno4dExmwTOrAieCHHiI99KoQsFil2rh3nn0wkZn3R119ImwJw2gqDSzf9
# EGe6esgyhmlvcLM2gfJaJJ/3v028fgmoARZ2gqXceXNIG8nhXfk0ORORyitraWWB
# 24xL0dnKdj/c/AiYCMoa9SZSobipfeUPAmE1fvwznRfjg29iSo+gzrzEYfM1djME
# OvRTHUXoZZRLn1QIfDsN7foo859ZvHvWnWSSKbpRn6qzPSvXZDVqy0VXGwmvN4m9
# nww=

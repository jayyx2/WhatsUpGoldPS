<#
.SYNOPSIS
Retrieves interface discard reports for devices within specified groups from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceGroupReportInterfaceDiscards function fetches interface discard reports for all devices within the specified group IDs from WhatsUp Gold's API. It supports a range of filtering options, including time range, sorting, grouping, thresholds, and pagination. It also allows for the inclusion of devices from sub-groups within the specified groups.

.PARAMETER GroupId
An array of group IDs from which to fetch interface discard reports. This parameter is mandatory.
...
.EXAMPLE
PS> Get-WUGDeviceGroupReportInterfaceDiscards -GroupId @("1", "2") -Range "lastWeek" -SortBy "avgPercent" -SortByDir "desc" -Limit 50
Fetches last week's interface discard reports for devices within groups with IDs 1 and 2, sorted by average percent in descending order, with a limit of 50 entries per page.

$Cred = Get-Credential
Connect-WUGServer -serverUri 192.168.1.250 -Credential $Cred
$groups = Get-WUGDeviceGroups
Get-WUGDeviceGroupReportInterfaceDiscards -GroupId $groups

.NOTES
Author: Jason Alberino (jason@wug.ninja) 2024-03-31

Reference: https://docs.ipswitch.com/NM/WhatsUpGold2022_1/02_Guides/rest_api/#operation/DeviceGroupReport_DeviceInterfaceDiscardsReport
...
function Get-WUGDeviceGroupReportInterfaceDiscards {
    [CmdletBinding()] param (
        [int[]]$GroupId,
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAWJh/mIBY8bm+l
# xozoIbg8FKoSDCO1QeNASUvXY/jxJKCCEfkwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BgkqhkiG9w0BCQQxIgQg+ZCOj4FnmuIuwJzNptOIPc8OyC0+h71WtdZjo2ITktkw
# DQYJKoZIhvcNAQEBBQAEggIATaJ5XffUuox6FOTLIOGEZ+9k54B47Ovba5yIy0il
# +AnmI93BcTxVNvZWfZ2uz21TWrf0iNrC6pkg3nxzcPsXktuIZgVyrdi01Xaa1OlP
# 1f70+fXei01yJIhiQ0tjvzESuIA8/z3ofzLkoZG0E4v0bcR22Xepf0ljX4RAtgO1
# SPMhYmKASG0jHeh/6E264Y/ZEexIWkoLXG57YLVo0NlKykfIjbH5PrOgmTedusIK
# t97ez+Qo6Kwk7JoHWiYIGgWj6kIKUklTS031kADdR5o1ta591CZOj99YAPTbM12V
# 5dRkFrB2QrlCIWbgb9V/FFFSlFy7V9WkkHzs/3GkIYuAjb9sqNzObjtFGFGod6oG
# GeSgxFpp88/Kgc6eEf/KFAFBViCWSx1TU7wLRtwIkKDexOGOVi8bqVYLVYyku77c
# bYHfUjRWSyXaqxjCg206AcOvbLwh+LViDIoF12pRh3HblRG4ugo/c2gGBLTHst5R
# ZjVN0VBsZxvLoasvYiGvsXsLDGmsBz+TM5eK7fH4azfZxilkk6vMTPcL7aURLRIJ
# miBYcd/PG/MK24k8SncYwzTdepMLjxUNTKG184PqRHltXC9NavDfSZEt+o8jvdPw
# DICT6/n2q0HenobHVnl453+FksrrO6e+c0YfAjh/tWmhZd7oJI1Uzs7Ra87zE+NL
# k9k=

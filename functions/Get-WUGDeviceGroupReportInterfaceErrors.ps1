<#
.SYNOPSIS
Retrieves interface error reports for devices within specified groups from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceGroupReportInterfaceErrors function fetches interface error reports for all devices within the specified group IDs from WhatsUp Gold's API. It supports a range of filtering options, including time range, sorting, grouping, thresholds, and pagination. It also allows for the inclusion of devices from sub-groups within the specified groups.

.PARAMETER GroupId
An array of group IDs from which to fetch interface error reports. This parameter is mandatory.
...
.EXAMPLE
PS> Get-WUGDeviceGroupReportInterfaceErrors -GroupId @("1", "2") -Range "lastWeek" -SortBy "avgPercent" -SortByDir "desc" -Limit 50
Fetches last week's interface error reports for devices within groups with IDs 1 and 2, sorted by average percent in descending order, with a limit of 50 entries per page.

$Cred = Get-Credential
Connect-WUGServer -serverUri 192.168.1.250 -Credential $Cred
$groups = Get-WUGDeviceGroups
Get-WUGDeviceGroupReportInterfaceErrors -GroupId $groups

.NOTES
Author: Jason Alberino (jason@wug.ninja) 2024-03-31

Reference: https://docs.ipswitch.com/NM/WhatsUpGold2022_1/02_Guides/rest_api/#operation/DeviceGroupReport_DeviceInterfaceErrorsReport
...
function Get-WUGDeviceGroupReportInterfaceErrors {
    [CmdletBinding()] param (
        [int[]]$GroupId = @(-2),
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDMQpBo6eDETmE8
# 1NAYtIuogEnRJKyO58nDdT8lOjAnQaCCEdMwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgkcMxRoVW10BFGFUHPyPc2l0Ds+YhHkfi
# 9EpGXfHLtxIwDQYJKoZIhvcNAQEBBQAEggIAK5czLuOz9qGLf04U1zhQrfnH1OWS
# 1S5+oM/JgjmK9jxBxV1uJewYATIQ0xA+9BslZm6cgIXvg59LKvzvwnkApNtgY3ZO
# XyE64mU20sEAH26VObQJ/yK6C4GDmMkuLrBqmidhhaRNWRAWqg6yT6MmezX4eyGg
# H+Sa+QbyttQFW40nsJinrWJtiGQejRpP3O7aVcQieErDpqb9H3tpLhXcF9pObhZW
# Xv7rKb39u9NQZCCgQn5ug7tenNMZLqK6ylHGU1lGoH6NHStDuA+Yv+0r7gjzaNIk
# 4QhSDxxKzLSi7uFyvVom/YI2YoCA0Bz/cXmatQADpweRo0T+atF1E+RrUIx0IbtY
# lCcBSSWSMz0u7X7Dx1ApqxG+dpAfzmsNNO/k7NKsPhiQenUJaUTb6rHaDy32Wl0D
# br5fRHuuERoCYfWMO6/sXpHa9hVYRmhyrdHRX8nX5M/0DSaJrPvahBYPIxVgJOoE
# eGpvAHzR9bI92CvZ6VfGCqwtHaUGmq3Kx5gOm21lsxz3ErUvRKWl1ClfO1vOY2Fy
# Vyqu9edSnPFyr7YPcgUI7fDdyQzQH5ABtONwgOmphZ8U2Ki00SuV8zF79awbdSnp
# qOkQiRyH890pql/L9e0MokSCgXd6BvGQ2ngQTHhkjOhnyvQHaF3j2EkulAz8XKFk
# U1oEM5XlkAN5kyU=

<#
.SYNOPSIS
Retrieves ping availability reports for devices within specified groups from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceGroupReportPingAvailability function fetches ping availability reports for all devices within the specified group IDs from WhatsUp Gold's API. It supports a range of filtering options, including time range, sorting, grouping, thresholds, and pagination. It also allows for the inclusion of devices from sub-groups within the specified groups.

.PARAMETER GroupId
An array of group IDs from which to fetch ping availability reports. This parameter is mandatory.
...
.EXAMPLE
PS> Get-WUGDeviceGroupReportPingAvailability -GroupId @("1", "2") -Range "lastWeek" -SortBy "avgPercent" -SortByDir "desc" -Limit 50
Fetches last week's ping availability reports for devices within groups with IDs 1 and 2, sorted by average percent in descending order, with a limit of 50 entries per page.

$Cred = Get-Credential
Connect-WUGServer -serverUri 192.168.1.250 -Credential $Cred
$groups = Get-WUGDeviceGroups
Get-WUGDeviceGroupReportPingAvailability -GroupId $groups

.NOTES
Author: Jason Alberino (jason@wug.ninja) 2024-03-31

Reference: https://docs.ipswitch.com/NM/WhatsUpGold2022_1/02_Guides/rest_api/#operation/DeviceGroupReport_DevicePingAvailabilityReport
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBJU90IFnwoMXri
# dcapFzR2o2Yc/W01E2txN19KrBMMrKCCEdMwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgrn03Et9PPfuuOD2EQcyv4spUd+rdJida
# UttpZ5IeuXUwDQYJKoZIhvcNAQEBBQAEggIAN1pff2/Pafni/x4yo2YAXwbhLs5P
# XkRbEdXPafH/TDN/Boet8ma8qzOd42K4eJwg4gDMySp3+rTJedTrNt0+GXrMHvf8
# 2VR+DYqqYNDKwa0InZuLCzU4s6azJawhHVbmH4eUBg47aoBdv0DRBoEBHHMNV+de
# H109IDhbGuIDdTny1M8cFi31+1/gdoff0MHrLM/mJKX7ENj3Q6HdC8hoCcM2910X
# eMEufQHRMWqBZqLa+njcb0DEng0ZSxAEF40Aq3CAnQlGessKCVH+M0RUZgbOBxqn
# 2ZT9fHMPj8zQ6GFqon1IZcNvOuC+3QaOjd/2QNhDp20aIFzEvFfDYSlKOJs8JB2P
# mRWTHUAprEN5ADXQEYz1DW0oz8UQ3XQ/zfspKxdNC1+IrSvI/DBBJyh8GPqm3iL5
# ywbgY9N479JR7kI0UxXKD0x6xdifLK0sNzpM1I1zyCRgnHANuz/vhoTXDvVWFPiQ
# LlpFJZq67iRhgBPRjDyf1UUNy5Mtw+4u8EgSwCZa4vm9cD7+0w4pbZh9yUAJKyJh
# B5n76w9CndATF8yJM/IwC0UgwKbFKBFjQYz5/XRpy2f33KlpANzkHhZKYehSZ/p4
# DwN5pqlad0FGMY5Cf0zZswO7v3IiajMd73XnAKhK3r7VcGWv12ndMh/9kqd45wht
# FvsySrujrlmVTG4=

<#
.SYNOPSIS
Retrieves ping response time reports for devices within specified groups from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceGroupReportPingResponseTime function fetches ping response time reports for all devices within the specified group IDs from WhatsUp Gold's API. It supports a range of filtering options, including time range, sorting, grouping, thresholds, and pagination. It also allows for the inclusion of devices from sub-groups within the specified groups.

.PARAMETER GroupId
An array of group IDs from which to fetch ping response time reports. This parameter is mandatory.
...
.EXAMPLE
PS> Get-WUGDeviceGroupReportPingResponseTime -GroupId @("1", "2") -Range "lastWeek" -SortBy "avgPercent" -SortByDir "desc" -Limit 50
Fetches last week's ping response time reports for devices within groups with IDs 1 and 2, sorted by average percent in descending order, with a limit of 50 entries per page.

$Cred = Get-Credential
Connect-WUGServer -serverUri 192.168.1.250 -Credential $Cred
$groups = Get-WUGDeviceGroups
Get-WUGDeviceGroupReportPingResponseTime -GroupId $groups

.NOTES
Author: Jason Alberino (jason@wug.ninja) 2024-03-31

Reference: https://docs.ipswitch.com/NM/WhatsUpGold2022_1/02_Guides/rest_api/#operation/DeviceGroupReport_DevicePingResponseTimeReport
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBCSMJr41MKuCVm
# mo6aeys+CAp0vC9BsBvyBbJKfsfcq6CCEdMwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgFO3JyYjIyr+xl9SKUeN4sOwPl+pT0dN9
# 4BgNqrR1uLwwDQYJKoZIhvcNAQEBBQAEggIAiVA6c427GJhvYqw6GQm8k2lPuwgx
# yl2g2DL0nTj47j2MFsbCSQhcZ98mFnZaQUPS2ntlLez90lKj3a3JUB6XvagzOmrR
# VoJN1EmtTfRLLK+7PjEkOa4GHrVaKUiibyQS5jRb105ORb4dD51gBVPPLlY/aZOo
# 08CXG1XOx8i1HTDIBKGfPmaddCjLYNwEzuQ4eru7enrHiMNN8SYrzKlr6A3iUDCa
# ExqnJ/1zYAqX3+fAhZjj3rSBPfwanCK5G7flhNt+6gbHsgkQxKMjiCE8CnHpx+oz
# uy1r3sMGKVveNJME1QENxOxF8PrxxMuwSiC3EChEwxeLm9HKMJEDIX7+gDBZ7459
# 6577y1U8xb7zPW6vAZ74OsCW8qiCkhfBt9KYXUine81Tm3xycvNq5dp5D/SuyRs9
# v8bFRE1wHUQT9AAnGgv4WPrV+60hYv5618tIezOwEGlgT247190zODmalJ5rDN6Q
# jnMFsrr8e4DMb2EAUbH7agXKhdPeKJJFshMwp+qZXUcFhPMW9QEMERwoD36T7UVt
# LNnfcXF8lwSlGK2+5KiIHvrTKr1+KpfaTBFqoWkI09cKmxO2rriA0O0wUZ4EFY2Q
# 0WTVxqPZFYk8444qIyxGf9yuWU0vb/YC6idRi4wRn3kT6xORVmbNy+bL7j1n3pdh
# aBuNao2t2La2jSE=

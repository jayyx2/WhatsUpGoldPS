<#
.SYNOPSIS
Retrieves state change reports for devices within specified groups from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceGroupReportStateChange function fetches state change reports for all devices within the specified group IDs from WhatsUp Gold's API. It supports a range of filtering options, including time range, sorting, grouping, thresholds, and pagination. It also allows for the inclusion of devices from sub-groups within the specified groups.

.PARAMETER GroupId
An array of group IDs from which to fetch state change reports. This parameter is mandatory.
...
.EXAMPLE
PS> Get-WUGDeviceGroupReportStateChange -GroupId @("1", "2") -Range "lastWeek" -SortBy "avgPercent" -SortByDir "desc" -Limit 50
Fetches last week's state change reports for devices within groups with IDs 1 and 2, sorted by average percent in descending order, with a limit of 50 entries per page.

$Cred = Get-Credential
Connect-WUGServer -serverUri 192.168.1.250 -Credential $Cred
$groups = Get-WUGDeviceGroups
Get-WUGDeviceGroupReportStateChange -GroupId $groups

.NOTES
Author: Jason Alberino (jason@wug.ninja) 2024-03-31

Reference: https://docs.ipswitch.com/NM/WhatsUpGold2022_1/02_Guides/rest_api/#operation/DeviceGroupReport_DeviceStateChangeReport
...
function Get-WUGDeviceGroupReportStateChange {
    [CmdletBinding()]
    param (
        [int[]]$GroupId,
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUvxDhvrzfnBhMwcPQm4tjvwIL
# e3mgghH5MIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUesfuRrF1zMu/gXH1cyH/vYZO
# o/cwDQYJKoZIhvcNAQEBBQAEggIAAM78zMbsdDbSL5FcnCWzdzzgHjHTQDOa228n
# sthAuU57XVoFnlYJMF+NFDJ7Ca+vsgRZOF3qyj7pZQGUXGFSRkbANNz+bIzKLXeP
# ClKQL2G8QJWLxaW5KXWh09P8DjpGla4lkVUwRPx+T9ji4WuOvQiyGI/4N0tg97fd
# nny4UzaXvpVhZshhZuS+4R5Oi1dkgv/lI1qcSjtWr9L/QQQdtWr3w7JEFOTyIMAS
# S/4eN+4shiYcyDx4OCMEzUh8JYX5ksLTuvsKU0g1nwBPFiBEUVlnteFGoPqwiB/F
# HC2Hr7UR7MzXa7WE/fdK7b6k8bfzoTv3V+y+2WwBSKURVTkMVnIdGG27nwRwRIUC
# YJBzxiAR+Vge2fQAJkDqCWlXNeVQuXTxCJAIXwG+KGhyYXsHIE8yn+qwNXW63Aij
# LQsFMcTYxmsru1cjZcOqW8IwNXvKHbSshrfAyWMIGRqCFK2rhyB6R6Pkylkvg0iV
# LpleAMlv5aiByRtK9ptYB3vtqk1NzRR6xTCmaGJ3MWMuYMgZ/8nCHJ6KDONHraOY
# 13IKvVZvlkWFWUcvC09ibtzEzh6fNtLmlJcu0VwndSC1zPOCRlwbA2FLhjhPsjQ/
# 8aKh1vV+wv3iGkXs8sphqp0xDSqBSEhFYn887GmrSI3MjeAzREPUUbpZKHgs+1U4
# Jpg2lC8=

<#
.SYNOPSIS
Retrieves Interface traffic reports for specified devices from WhatsUp Gold.

.DESCRIPTION
The Get-WUGDeviceReportInterfaceTraffic function fetches Interface traffic reports for a list of device IDs from WhatsUp Gold. It supports filtering by time range, sorting, grouping, applying thresholds, and automatically handles pagination. The function provides progress feedback during execution.

.PARAMETER DeviceId
An array of device IDs for which Interface traffic reports are requested. This parameter is mandatory and accepts multiple values.
...
.EXAMPLE
PS> Get-WUGDeviceReportInterfaceTrafficUtilization -DeviceId @("1", "2", "3") -Range "lastWeek"

Fetches last week's Interface traffic reports for devices with IDs 1, 2, and 3.

.EXAMPLE
PS> Get-WUGDeviceReportInterfaceTrafficUtilization -DeviceId @("4") -Range "custom" -RangeStartUtc "2023-01-01T00:00:00Z" -RangeEndUtc "2023-01-07T23:59:59Z"

Fetches Interface traffic reports for device 4 for the first week of January 2023.
...
#>
function Get-WUGDeviceReportInterfaceTraffic {
    [CmdletBinding()]
    param (
    ...
    process {
        ...
        if ($totalDevices -eq 0) {Write-Warning "No valid DeviceIDs provided."; return;}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUvrzVk2NyvW7vZ756XCWEgnJb
# qiCgghH5MIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# DAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUCuLgv2kDrUn4tAajW2tKAs6+
# XxswDQYJKoZIhvcNAQEBBQAEggIAGIn1DvTv4pzxkVORmQiNHTFmub3P224T3ZHX
# 89zLMPIBTVPPkWpwbMFZlOTr0CQAhJ+93hT3W6EiOPB1PkcXn9hiXt9z0UYQHVks
# SX4YOMmt1XlchEVljEEmdELfip+c6Vsh0CQydhES0AglxyW6gJqZokx/fxKD8tDj
# 5gZu50qVZTUZnB10DDipmT2cBI8xbXoMJ9ck2X7ncmlbrv30HKKW6k9hbKHu68VR
# BU+ip+5aZ2ic8KMbCw7mOB+ALBRLMDOjScyEE2Ov4l2DwOeIo+Jc5jEUAPVEhAQu
# 7ytlfrzIjmwS9Mnut3qqW5zpSB8QIxS0ntQ5Mn3MuwLwQ1wuYzlJ42KFp2XBqMRF
# jY8vhijh8NTlZ2aRlyXCO+H+sfWkRO+gP7iDifUj1WWBGypjSqOObJWyaHWgyMXs
# 3gj6ophBg72Gry1yAYk96K+AmnlM9oCarVfDhaq0131z6ccb1bHwz5NaMmRFOtA6
# 8emaL4xWDTmz7Gj3D+NpHagIpuiyG13pGlbsn0JiUdRtzisqtFBtGpis4Zpqkw+W
# e5OMtQyfuwl/KTUx8IsKzF7CpsMjJFKcVnX2g6P2NRsfOURM1SIkPojnnM7kKn+6
# 3llQTkhnpqBXI3KlgWsfXI6KggYsGQamwLPx+mFKdrc9UA2M/aArfuR5wsjGE6ND
# UxlDcig=

function Invoke-WUGDeviceRefresh {
    ...
    param (
        ...
        [ValidateSet("true", "false")][string]$UpdateNamesForTableActiveMonitor,
        [ValidateSet("true", "false")][string]$UpdateEnableSettingsForTableActiveMonitor,
        [ValidateSet("true", "false")][string]$AddUseInRescanActiveMonitor,
        [ValidateSet("true", "false")][string]$IncludeAssignedRoles,
        [Parameter(ParameterSetName = 'ByGroup')][ValidateSet("true", "false")][string]$ImmediateChildren,
        [ValidateSet(
        ...
        [ValidateRange(-1, [int]::MaxValue)][int]$DropDataOlderThanHours
    )
    ...
    process {
        ...
        if ($PSCmdlet.ParameterSetName -eq 'ByGroup') {
            # The group endpoint takes its options as query parameters, not a body.
            $q = @()
            if ($ImmediateChildren)                        { $q += "immediateChildren=$ImmediateChildren" }
            if ($UpdateNamesForTableActiveMonitor)         { $q += "updateNamesForInterfaceActiveMonitor=$UpdateNamesForTableActiveMonitor" }
            if ($UpdateEnableSettingsForTableActiveMonitor) { $q += "updateEnableSettingsForInterfaceActiveMonitor=$UpdateEnableSettingsForTableActiveMonitor" }
            if ($AddUseInRescanActiveMonitor)              { $q += "addUseInRescanActiveMonitors=$AddUseInRescanActiveMonitor" }
            if ($IncludeAssignedRoles)                     { $q += "includeAssignedRoles=$IncludeAssignedRoles" }
            if ($ResetOptions)                             { $q += "resetOptions=$($ResetOptions -join ',')" }
            if ($PSBoundParameters.ContainsKey('DropDataOlderThanHours')) { $q += "dropDataOlderThanHours=$DropDataOlderThanHours" }

            $uri = "${global:WhatsUpServerBaseURI}/api/v1/device-groups/${GroupId}/refresh"
            if ($q.Count -gt 0) { $uri += '?' + ($q -join '&') }
            ...
            # PATCH /devices/-/refresh only rescans the last id in the list, so each
            # device is queued through the per-device endpoint instead.
            $options = @{}
            if ($UpdateNamesForTableActiveMonitor)          { $options.updateNamesForTableActiveMonitor = [System.Convert]::ToBoolean($UpdateNamesForTableActiveMonitor) }
            if ($UpdateEnableSettingsForTableActiveMonitor) { $options.updateEnableSettingsForTableActiveMonitor = [System.Convert]::ToBoolean($UpdateEnableSettingsForTableActiveMonitor) }
            if ($AddUseInRescanActiveMonitor)               { $options.addUseInRescanActiveMonitor = [System.Convert]::ToBoolean($AddUseInRescanActiveMonitor) }
            if ($IncludeAssignedRoles)                      { $options.includeAssignedRoles = [System.Convert]::ToBoolean($IncludeAssignedRoles) }
            if ($ResetOptions)                              { $options.resetOptions = $ResetOptions }
            if ($PSBoundParameters.ContainsKey('DropDataOlderThanHours')) { $options.dropDataOlderThanHours = $DropDataOlderThanHours }
            $jsonBody = $options | ConvertTo-Json -Depth 4

            $scanIds = @()
            foreach ($id in $DeviceId) {
                $uri = "$baseUri/$id/refresh"
                Write-Verbose "PUT $uri"
                try {
                    $response = Get-WUGAPIResponse -Uri $uri -Method 'PUT' -Body $jsonBody
                    if ($response.data.success -eq $true) {
                        Write-Verbose "Refresh accepted for device $id. Scan ID: $($response.data.id)"
                        $scanIds += $response.data.id
                    }
                    else {
                        Write-Warning "Refresh request for device $id returned success = false."
                    }
                }
                catch {
                    Write-Error "Failed to refresh device ${id}: $_"
                }
            }
            return $scanIds
        }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCU30Zf8Ngz5hQC
# gS/FJ7DLQ3/vAhMxN4W2BygvUgmtd6CCJQ0wggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BCCG2FulAOV2soy04PWfMip+KgQGSddP9UahCt4kwxNFDDANBgkqhkiG9w0BAQEF
# AASCAgCcAHbw5s8QMMMub5J+erKbbAefsI0+JTs3HqA7eyQl7xM6ftMl+KX/yg/k
# Ii+SaVEdtjb0DKSB/zOAZrHzQiM+VACgCd/rLhORENf74rQGMkOZfeHmb7rlz7pL
# hekpNiqKXdEncTkAbeu/OJqMGdJTdZfyU33GOqukasyD4TY6r8WrvFqg4sARqSxi
# V7D1alI3KylP/rVCnF0jUzH2K30Ni3YEQP1z4DeN2OEU6vJ5zc0iWq3MbRncCqGh
# lsgCjrNLn2+1iGSS1KnDm5ta7wTdpZEbLyARU3ldHYUCsGM2fAc+onfaYv6VCS8t
# rH90RVv04WYzxmPGBHLbRCsYOjtXKyRvk4HHRzheSXME40oJAf4h8LTjWmMLvdkp
# wBK3vSFz2Qsxx6yAGKugxf8IcqFx3dmwjanqQHbtq+1HH/H1PSmxyTjJCTFuGkbO
# 1c6dJBeo2mDPu0DeM9rnp0NokBLPztOOKLMuJbhWcj2k372H+1ylJLnvIeNQijCf
# W+qf7zc1sp+puJjQ3s1K3zbickruOM6FhKE89iRrXPL/LWgNdDwsQ8HeLjaRGjm+
# 79i4hUA+Wja5UVL5yVM990Nc59jKOEaQAFWsTWzIDdI4PjRc3z3M1xwaLY7ffHjO
# klVHOJAoWPeVwruwB13Ml43cdBS4Sv1///UNmIx3tmMb6Qs55KGCAyYwggMiBgkq
...
# CSqGSIb3DQEJBTEPFw0yNjA5MjAxNTMwMjFaMC8GCSqGSIb3DQEJBDEiBCBKPRPv
# RREnxWahAVziloeem/W3gaVhWm76eGBLIdXRnzANBgkqhkiG9w0BAQEFAASCAgB8
# u/m239AUK5gC3VNScW3QZyFrwqIW2aoCMOmwInYeNVWl9AxvmYu+xDiQ3kL7zJZ/
# epLQFH0kqnoFgop1NpFih1IADxwIoU9iG9ksHtxSPXGlQ5iWK6Gmd8ymLoFm49dx
# 6kfOhxu3iXqqmH4gxX0tKsKsHaHYi59NEPPfVcfAh38LtQgNEEmYQUmnRZSK6bZN
# XIKITqYS4ihuwZ9tcOesYo+pLjUWYPQNKTLRBSpl7PEPHM2gzOqwUvjsTc6CJ0+X
# gofWx59ws7gOi1EgCMhTWyS24tIMSrxpl4nphWVAsO+ZdSCXnm2qXhVqQqmQzu6a
# fJXF7Lf1USgYM6p0Az8f6i46RJntSxHCgjv/5jpqlF7w+A81UffmqODrDzdgbncU
# QZ4kYMVlQAOnrfRO0ai45bVB/AycYjmgvUXjWn+MDtIds5LbPa7bUCgNQAw9teqf
# WIgD8xuWyqP2YXe1i6XYAPjTy/+WfZO6ujr2eWEECIU7fLnFq8fP0zSupyip6TUY
# a6rVEoboX8LqB6/To4C89MsMTyrs4ECSjx13Kv4nNLkT/uf5KC0KIB0jnt5AawF8
# X52ltMaru9K+xXiOrdkR+sFtleDNmPea0EBXpXAGUsa4X5RB+qlgRzz0l4rcjall
# +/vqqpEpBPA6qwjJN5Ugi4z3XQAW4dYtR695qmGUAQ==

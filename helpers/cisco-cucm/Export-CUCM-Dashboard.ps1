...
if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $SummaryDirectory 'dashboards'
}
...
$functionPath = Join-Path $WhatsUpGoldPsRepoPath 'helpers\reports\Export-DynamicDashboardHtml.ps1'
if (-not (Test-Path -LiteralPath $functionPath)) {
    throw "Dashboard function not found: $functionPath"
}
. $functionPath

$templatePath = Join-Path $WhatsUpGoldPsRepoPath 'helpers\reports\Dynamic-Dashboard-Template.html'

Write-Host "=== CUCM Dashboard Export ===" -ForegroundColor Cyan
Write-Host "Input: $SummaryDirectory" -ForegroundColor Gray
Write-Host "Output: $OutputDirectory" -ForegroundColor Gray

$phoneJsonPath = Join-Path $SummaryDirectory 'CUCM-phone-inventory-summary.json'
if (-not (Test-Path -LiteralPath $phoneJsonPath)) {
    Write-Warning "Phone inventory not found"
    return
}

Write-Host ""
$phonesRaw = @(Get-Content -LiteralPath $phoneJsonPath -Raw | ConvertFrom-Json)
$phones = $phonesRaw
if ($phonesRaw.Count -eq 1 -and $phonesRaw[0] -is [System.Object[]]) {
    $phones = $phonesRaw[0]
}

Write-Host "Loaded: $($phones.Count) phones" -ForegroundColor Green

if ($phones.Count -eq 0) {
    Write-Warning "No phone data"
    return
}

Write-Host "Generating dashboard..." -ForegroundColor Cyan

$outputHtml = Join-Path $OutputDirectory 'cucm-dashboard-phones.html'
$outputJson = Join-Path $OutputDirectory 'cucm-dashboard-phones-data.json'

($phones | ConvertTo-Json -Depth 20) | Out-File -LiteralPath $outputJson -Encoding UTF8

$reportData = @(Get-Content -LiteralPath $outputJson -Raw | ConvertFrom-Json)

# Unwrap if needed (ConvertFrom-Json wraps arrays)
if ($reportData.Count -eq 1 -and $reportData[0] -is [System.Object[]]) {
    $reportData = $reportData[0]
}

Write-Host "  ReportData count: $($reportData.Count)" -ForegroundColor Gray

# Export-DynamicDashboardHtml is not strict-mode safe; disable before calling
Set-StrictMode -Off

if ($Offline) {
    $reportData | Export-DynamicDashboardHtml `
        -OutputPath $outputHtml `
        -ReportTitle 'CUCM Phone Inventory' `
        -StatusField 'Status' `
        -ExportPrefix 'cucm_dashboard' `
...
if (Test-Path $outputHtml) {
    $size = [Math]::Round((Get-Item $outputHtml).Length / 1KB, 2)
    Write-Host "Dashboard: cucm-dashboard-phones.html ($size KB)" -ForegroundColor Green
}

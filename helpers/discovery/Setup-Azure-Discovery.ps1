...
if (-not $TenantId) {
    ...
    if (Test-Path $vaultDir) {
        ...
        if ($azCredFiles -and $azCredFiles.Count -gt 0) {
            # Extract TenantId from filename: Azure.<TenantId>.ServicePrincipal.cred
            $credName = [System.IO.Path]::GetFileNameWithoutExtension($azCredFiles[0].Name)
            $parts = $credName -split '\.', 3
            if ($parts.Count -ge 2 -and $parts[1]) {
                $TenantId = $parts[1]
                Write-Verbose "Auto-resolved TenantId from vault: $TenantId"
            }
        }
    }
}
if (-not $TenantId) {
    if ($NonInteractive) {
        Write-Error 'Tenant ID is required for non-interactive mode. Pass -TenantId or ensure Azure credentials exist in the vault.'
        return
    }
    Write-Host ""

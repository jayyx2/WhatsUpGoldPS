...
foreach ($project in $GCPProjects) {
    ...
    try {
        ...
        foreach ($vm in $vms) {
            ...
            if ($ip -and $ip -match '^\d{1,3}(\.\d{1,3}){3}$') {
                Write-Host "    $($vm.Name) -> $ip" -ForegroundColor DarkGray
                $allIPs += $ip
            } else {
    ...
    try {
        ...
        foreach ($db in $sqlInstances) {
            ...
            if ($ip -and $ip -match '^\d{1,3}(\.\d{1,3}){3}$') {
                Write-Host "    $($db.InstanceName) -> $ip" -ForegroundColor DarkGray
                $allIPs += $ip
            } else {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDr8InUboIU9YtI
# qmcrP4jnVk24vZ3/TSPhvJCrBhWWzqCCEdMwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# BAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgEghtAWRkFvcCWxxmnCaL7BpP1Ahx16qV
# zGD7xqzlCfcwDQYJKoZIhvcNAQEBBQAEggIA18SAoVy2YxtIguu/1j5YQZB6iU4C
# p7iLjKLwvUrx+IUxiR18e1xApfLtf+zSUf4l7OZPdwxyz8/gQvg3YKIPLUcKx6dO
# CCDfjwdL+207Ns0gqkAOPS5Eb19iXFHXiarrj3byMwYHqTRPwmkNmNXM+RjG7AYE
# 7YECvIWS0uSqmwINeBdMLxaUyXyn9nVqjnJtCjINbjPxL3dn08ThG00PFLOt1Nyg
# mvvpxLiVY72/NZ5mziI8vLtEExwTyZPHUaG84X2LRuoqx0PNFUf/NdrAstfFG6GV
# 4sUeMoSTWzi/9XhkHnLgU9JCHJL3l5WcMc2PRZpiy0U13+MzQ3jixivM/PS0UiaG
# MQKW+1yN3rAaKQosnDBiTnsdOC1a3lIt8LJBJ5f4+ev+gi6gUpQ22wQg2xnauKm2
# EIvyhoQXWtMF+ii4dLOvDqrdjl9IOvhGZ+wgN5MD8VBI2ytJ3jVMjZdB8MhHhawX
# ADNH89MPlzLGAthtoblI26uA97eAx/gMo3bUMswLjjxSPC1E9OzfTGc6jKID2GV/
# k6zVFezJ1c/ZBqqKOVfH6rTMBeY+BDOHF/OpIsRyTCZHG8FflEcs/ip2ZQ5FPi55
# 9ZwyaKQ0m/VpP1p9AUJzAesEFpZ/RoeiTxMYl3nM2jTkvfQiH00Tu/Q8C50leAXT
# vXOy5u1daUZxPIE=

...
# -- Connect-WUGServer -AutoConnect (vault-based reconnect) -------------------
# The initial connection with -Credential saved to vault, so -AutoConnect should work
Invoke-Test -Cmdlet 'Connect-WUGServer (-AutoConnect)' -Endpoint 'POST /token (vault)' -Test {
    Connect-WUGServer -AutoConnect -IgnoreSSLErrors -ErrorAction Stop
    if (-not $global:WUGBearerHeaders) { throw "No bearer headers after AutoConnect" }
}

# Verify the reconnected session actually works
if ($global:WUGBearerHeaders) {
    Invoke-Test -Cmdlet 'Get-WUGProduct (post-AutoConnect)' -Endpoint 'GET /product/* (vault session)' -Test {
        $p = Get-WUGProduct -ErrorAction Stop
        if (-not $p.version) { throw "No version returned after AutoConnect" }
    }

    # Disconnect again for clean exit
    Invoke-Test -Cmdlet 'Disconnect-WUGServer (post-AutoConnect)' -Endpoint '(session cleanup)' -Test {
        Disconnect-WUGServer -ErrorAction Stop
        if ($global:WUGBearerHeaders) { throw "Headers still set after disconnect" }
    }
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBiZMceaf7kX0JZ
# PdazS7MQ4IajVXbZWMk92BfRmsE0j6CCJNcwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# CisGAQQBgjcCAQsxDjAMBgorBgEEAYI3AgEVMC8GCSqGSIb3DQEJBDEiBCANCyUR
# MAdRqjB0p4eaZfDiDimJkHlSsRUtV/OiGW2DyjANBgkqhkiG9w0BAQEFAASCAgBS
# D8a8BWB9ZnXCvgI2B7pHkc5corsnH5eFl0r503e/V2jKCnz+zAKZxXbxIkfDSxQc
# cUAOz06o1wlkcEnemqJvYqLa5l2nY9/r7H8IwAj9MSZKVoo0+tBapjzWER7CjnHX
# d+G5njDxWe/HiKnpkLoX7cSfqt/R5y69nnQZ5bnPYZdr6mlfUGPKDTqBcxwkyij7
# Wsirj6E0txPAP7NztUPfblLZXNA2vl/MTRx20+G6ehnfGO/5KN9S7TSP446+w/Cb
# Y6iyCcQJqQ6qOfYLxf6IujqlmyY6Ewi7CV4AZN4FneOacWL42EAKWxyGEPvniqJ7
# QzLOfX5579Gzf3ftEt3TKyaobNeC+t3VakS0NmpydES5/ersuYnUI0udII2IMm1o
# ULeDLeq0gtsZD4mluJE0/t1Gp7simVFJE+Koqy1t2OjnglbORrShKRUl2PH8zJZP
# 7C3IBZhIaDjKFYpiIEGi+caWG1h0MX1EX9z6wbBj6wMNjn+6uJ/mXQBZyhbVc3CO
# BDYzllKzhzBDDFO8fnCiE2QeTrOGBVb8pe/ObmfKVivdUGnTKgfmQaSD8DLB8b7Z
# jlR2ODNXeqbq8EWy6ZVmN5lfMUgu/nTtvHD+dhblUS99MlD7UD2I98RfKm+PwGK2
# abf6vzoBxQ+bcHLSSiB/Da4kxdwtuazPYYMWWvMZqqGCAyMwggMfBgkqhkiG9w0B
...
# SIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI2MDQwNzE4NDAw
# OVowPwYJKoZIhvcNAQkEMTIEMModvHgrtaiKyNwXlx1JFdHVEE5laLypkHaIpAA+
# LyVBbqnaUpjG531YyQ5fgiiU9zANBgkqhkiG9w0BAQEFAASCAgBRAqDmmot0BwTW
# js4cCDqns25pzPq7w+EoGqJZsHEgyMLE3qPDXziMqoxbN7OI+20rCP1nWnbi0elw
# hCagxWeY200D205kYl/IpxR5Jrfc0BN/42Ra/32kdanNsGkmYe1zAbcku/NiXDsm
# cwBVHGvSVIBKQ9vBZSoUZSiOCR2ZeEyOrb5w/a0hYCpoUO6PYN4/5Vk4DBDYbYHA
# ADzh5zkt8urErTVIVeCHn8K1Cm5NsEyq5Aq0bMH/hHhIgQcJDEO8WXrIm6G+CP/H
# QJEAdGH0/eKdl/X8OVolLs9D5D/udWS9A5OTgasuRMEgMbl9CVTF4K9EEvCJ2oES
# wBbTLXXpaWJUtsHMnsX9lc/JGO/z8aboWvLKZKzJttJgpqSisDeBNQofyLagjjJG
# BnS6/DNbjbS0FsHP9V+4FmLLaoKRUEOkRpLNkA4deHSt+Uz1vHh5XH+A15iqTngU
# sYgYReikpQs3l+P0XfEYgXJ+EYaLpUAZp2zwoNMq/4lTwGYPZNIORmWJJBSP+yCH
# n9diZwu0CP/C3foi4dcfuj1D3tDOw4ac62Qz17tZkIVJQmWtIS3c2gpP4T7j1j52
# /2C5hZOFMM8HyIgaity8p1ZKtNBSG1+miNORUC5jiCg409V3rxD8fbkRKILfbh66
# lYLKxxMlOtYlxJlvgDOe2v4K+uLYxg==
